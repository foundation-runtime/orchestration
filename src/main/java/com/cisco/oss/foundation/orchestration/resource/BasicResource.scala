...
import com.cisco.oss.foundation.orchestration.{ScopeErrorMessages, ScopeConstants}
...
import com.cisco.oss.foundation.orchestration.provision.model.ProductRepoInfo
import com.cisco.oss.foundation.orchestration.utils._
import com.cisco.oss.foundation.orchestration.scope.ScopeErrorMessages
...
trait BasicResource extends Slf4jLogger {
  ...
  private def configureModulesPerStep(product: Product, modules: InstallModules, system: System, instance: Instance) {
    ...
    val ccpServersDetails = system.foundation.getOrElse(instance).machineIds.filter(set => set._2.hostname.contains("ccp"))
    // TODO: What to do with 2 ccp servers?
    val configurationServer = ccpServersDetails match {
  ...
  private def createAndDeployVMs(system: System, inst: Instance, deploymentModel: HashMap[String, HashMap[String, InstallationPart]], nodes: List[Node], stepsMap: Map[String, InstallModules]): Future[Instance] = {
    ...
    createVmsFuture onSuccess {
      case vmDetails => {
        ...
        val ccpServersDetails = newSystem.foundation.getOrElse(newInst).machineIds.filter(set => set._2.hostname.contains("ccp"))

        logDebug("Found configuration servers : {}", ccpServersDetails.keys)
        ...
        val vmHostsScript = NetworkUtils.createHostsFileScript(ccpServers, vmDetails)
        ...
        if (!ScopeUtils.configuration.getBoolean(ScopeConstants.CLOUD_ENV_HAS_DNS, false)) {
          ...
          if (!foundationProductName.equals(instance.product.productName)) {
            ...
            vmUtils.runScriptOnMatchingNodes(productVmScript.render(OsFamily.UNIX), s"edit_hosts${java.lang.System.currentTimeMillis()}", Some(foundationGroup), Some("ccp"), Some(List(foundationProductName)), foundationPrivateKey)
          }
        }
